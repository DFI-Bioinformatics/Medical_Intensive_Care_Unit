...
### set output folder:
```{r}
eddi.path = "~/Library/CloudStorage/Box-Box/Alexander_DePorto_Nick_Dylla/MICUpaper_Eddi_20240626/"

out.path = "~/OneDrive - The University of Chicago/clinical/20240620_MICU/"
```
...
first_samp_list_anon %>% 
  select(unique_id, ID, shotgunSeq_id, metabolomicsID) %>% 
  write_csv(file.path(out.path, "sampleList.csv"))

# Load in clinical variables
micu_new_anon <- readRDS("~/Library/CloudStorage/Box-Box/Alexander_DePorto_Nick_Dylla/MICUpaper_Eddi_20240626/micu_new_anon.rds") # new micu
...
metab_quant_imp_tot_mM %>% 
  select(metabolomicsID, compound, mvalue__mM) %>% 
  spread(compound, mvalue__mM) %>% 
  left_join(first_samp_list_anon %>% 
              select(unique_id, metabolomicsID)) %>% 
  relocate(unique_id) %>% 
  write_csv(file.path(out.path,"quant.imp.csv"))

metab_qual_imp_tot <- readRDS("./Data/metab_qual_imp_tot.rds")

metab_qual_imp_tot %>% 
  spread(compound, mvalue) %>% 
  left_join(first_samp_list_anon %>% 
              select(unique_id, metabolomicsID)) %>% 
  relocate(unique_id) %>% 
  write_csv(file.path(out.path,"qual.imp.csv"))
...
write.csv(
  tableone_nocovid_print,
  "./Results/Table_One_30_Days_Mortality_train.Eddi.csv",
  row.names = TRUE
)
...
write.csv(
  tableone_nocovid_print_clean,
  file.path(eddi.path, "Table_One_30_Days_Mortality_train_clean.Eddi.csv"),
  row.names = TRUE
)

# Import csv as dataframe
tableone_nocovid_csv <-
  read.csv(file.path("Results","Table_One_30_Days_Mortality_train.Eddi.csv"),
...
write.csv(
  roc_loop_df,
  file.path(eddi.path,"ROC_Results_Ridge_Cutpoint_train.Eddi.csv")
)
...
km_loop_df <- as.data.frame(do.call(base::rbind, km_loop))

km_loop_df2 <- NULL

for (i in seq(1,29,1)){
  km_loop_df2 <- janitor::clean_names(base::rbind(km_loop_df2, km_loop_df[["unadjusted.result"]][i][1][[1]], i))
}
...
# Combine plots
cairo_pdf(file.path(eddi.path,"/ROC_Loop_train.pdf"), height = 12, width = 14)
...
# number changed in Eddi's repeat:
  #              Survivor Non-Survivor
  # Survivor           91           13
  # Non-Survivor       11           32
  #                                         
...
coxauc %>% 
  gtsummary::as_tibble() %>% 
  writexl::write_xlsx(., file.path(eddi.path,"cox_model_SOFA_30_Day_Mortality_roc_loop_train.xlsx"))
...
coxauc_ecoc %>% 
  gtsummary::as_tibble() %>% 
  writexl::write_xlsx(., file.path(eddi.path,"cox_model_SOFA_Enterococcus_30_Day_Mortality_train.xlsx"))
...
coxauc_ebac %>% 
  gtsummary::as_tibble() %>% 
  writexl::write_xlsx(., file.path(eddi.path,"cox_model_SOFA_Enterobacterales_30_Day_Mortality_train.xlsx"))
...
coxauc_shannon %>% 
  gtsummary::as_tibble() %>% 
  writexl::write_xlsx(., file.path(eddi.path,"cox_model_SOFA_Shannon_30_Day_Mortality_train.xlsx"))
...
coxauc_mmp %>% 
  gtsummary::as_tibble() %>% 
  writexl::write_xlsx(file.path(eddi.path, "cox_model_MMP_30_Day_Mortality_train.xlsx"))
...
rms_vc <- rmst2(km_nocovid_vc$surv_days,
      km_nocovid_vc$thirtyday_mortality_overall_class,
      as.numeric(as.factor(km_nocovid_vc$grouped_md_score))-1,
      tau = 30)
...
write.csv(
  tableone_nocovid_print_vc,
  file.path(eddi.path,"Table_One_30_Days_Mortality_validation.eddi.csv"),
  row.names = TRUE
)
...
write.csv(
  tableone_nocovid_print_vc_clean,
  file.path(eddi.path,"Table_One_30_Days_Mortality_validation_clean.eddi.csv"),
  row.names = TRUE
)
...
coxauc_vc %>% 
  gtsummary::as_tibble() %>% 
  writexl::write_xlsx(file.path(eddi.path, "cox_model_SOFA_30_Day_Mortality_roc_loop_validation.xlsx"))
